...
<p><strong><?=$value['username']?></strong> on <?=date("d-m-Y H:i:s", strtotime($value['post_date']))?></p>
<p class=""><?=$value['post_content']?></p>
<a href="<?php echo URL ?>dashboard/fav/<?=$value['post_id']?>"><button class="ui info labeled button">Favourite</button></a>
<?php $fav = $value['favuser'] ? explode('|', $value['favuser']) : [];
echo '<button class="ui label">'.$value['favcount'].'</button>';
...
<p id="confirmation<?=$value['post_id']?>" style="display:none;">
	...
	<a href="<?php echo URL ?>dashboard/delete/<?=$value['post_id']?>"> Yes</a>, <a href="#<?=$value['post_id']?>" id="no">No</a>
</p>
<?php } endforeach;?>
